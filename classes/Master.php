...
Class Master extends DBConnection {
	...
	function save_budget(){
        ...
        if(!isset($_POST['department_id']) || empty($_POST['department_id'])){
            if(!empty($data)) $data .=",";
            $data .= " `department_id`='{$this->settings->userdata('department_id')}' ";
        }
	...
	function save_department(){
		...
		$check = $this->conn->query("SELECT * FROM `departments` where `name` = '{$name}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
		...
		if(empty($id)){
			$sql = "INSERT INTO `departments` set {$data} ";
			$save = $this->conn->query($sql);
		}else{
			$sql = "UPDATE `departments` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if($save){
			$resp['status'] = 'success';
	...
	function delete_department(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `departments` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
